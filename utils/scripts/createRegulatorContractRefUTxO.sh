#!/bin/bash
# mixaxim@mixaxim: ~$ chmod +x createRegulatorContractRefUTxO.sh; ./createRegulatorContractRefUTxO.sh
...
printf "\n"
printf "$RED%b" "<----------------------------- Create Regulator Contract Reference UTxO ----------------------------->"
printf "\n\n"
...
# Contract Address
echo -e "\033[1;32m[+] The Contract Address: \033[0m" "$MINTING_CONTRACT_ADDRESS"
...
cardano-cli transaction sign \
    ...
    --tx-body-file "$TX_PATH"/regulator_contract_ref.body \
    --out-file "$TX_PATH"/regulator_contract_ref.signed \
    --testnet-magic 1
...
commandOutput=$(cardano-cli transaction submit \
    --testnet-magic "$MAGIC_TESTNET_NUMBER" \
    --tx-file "$TX_PATH"/regulator_contract_ref.signed)
sleep 1
...
# Tx ID
TXID=$(cardano-cli transaction txid --tx-file "$TX_PATH"/regulator_contract_ref.signed)
