#!/bin/bash
# mixaxim@mixaxim:~$ chmod +x consumeCollateral.sh;./consumeCollateral.sh
...
printf "\n"
printf "$RED%b" "<---------------------------------------- Consume Collateral ----------------------------------------->"
printf "\n\n"
...
echo ""
firstDevOutput="$FIRST_DEVELOPER_ADDRESS + 2000000 + 10000 $HAPPY_TOKEN"
...
commandOutput=$(
    cardano-cli transaction build-raw \
        ...
        --tx-out-inline-datum-file "$FIRST_DEV_CLAIM_AT_PHASE_ONE_DATUM" \
        --mint="1000 $HAPPY_TOKEN" \
        ...
        --spending-reference-tx-in-redeemer-file "$PHASE_ONE_CLAIMIG_TOKEN_REDEEMER" \
        --tx-in-collateral="$collatTxIn" \
        --tx-out-return-collateral "$COLLATERAL_ADDRESS + 9000000000" \
        --tx-total-collateral 1000000000 \
        --fee 300000 \
        --spending-reference-tx-in-execution-units "(1, 1)" \
        --mint-reference-tx-in-execution-units "(1, 1)" \
        --script-invalid
    ...
    # --tx-in-redeemer-file "$PHASE_ONE_CLAIMIG_TOKEN_REDEEMER" \
...
echo -e "\033[1;32m[+] Tx Fee:\033[0m" "${commandOutput}"
